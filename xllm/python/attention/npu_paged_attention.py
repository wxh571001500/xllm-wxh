...
class NpuPagedAttentionBackend(AttentionBackend):
    ...
    def prepare(
        ...
        *,
        ...
        if metadata.q_cu_seq_lens is not None:
            self._actual_seq_lens: list[int] | None = metadata.q_cu_seq_lens[1:].cpu().tolist()
        else:
        ...
        if graph_mode and self._has_mha_layers and self._block_table_i32 is not None:
        ...
        if metadata.kv_seq_lens is not None:
            ...
            if metadata.q_cu_seq_lens is not None:
                self._mla_actual_seq_q = metadata.q_cu_seq_lens[1:].to(torch.int32).to(mla_device)
            else:
                batch = kv_seq_lens.size(0)
                self._mla_actual_seq_q = torch.arange(1, batch + 1, dtype=torch.int32, device=mla_device)
        else:
    ...
    def execute_mla(
        ...
        if topk is None:
            ...
            if metadata.is_chunked_prefill:
                return self._mla_dense_chunked_prefill(q_latent, q_pe, nope_cache, rope_cache, metadata, layer)
            if metadata.is_prefill:
                return self._mla_dense_prefill(q_latent, q_pe, k_latent_3d, k_pe_3d, metadata, layer)
        ...
        return self._mla_sparse(q_latent, q_pe, nope_cache, rope_cache, topk, metadata.block_table)

    def use_unabsorbed_mla_prefill(self) -> bool:
        metadata = self._metadata
        return bool(metadata is not None and metadata.is_prefill and not metadata.is_chunked_prefill)
    ...
    def _mla_dense_chunked_prefill(
        ...
        if len(actual_seq_q) != len(actual_seq_kv):
            raise RuntimeError("dense MLA chunked prefill requires matching query and KV batches")
    ...
    def _mla_dense_decode(
        ...
        q_latent, q_pe, fia_num_heads = _pad_mla_query_heads(q_latent, q_pe, layer.num_heads)
        if nope_flat.dtype != torch.bfloat16 or rope_flat.dtype != torch.bfloat16:
            raise RuntimeError("dense MLA paged caches must use BF16 for NPU FIA decode")
    ...
    def _chunked_kv_seq_lens(metadata: AttentionMetadata) -> list[int]:
        kv_host = metadata.kv_seq_lens_host
        if kv_host is None:
            raise RuntimeError("dense MLA chunked prefill requires host KV sequence lengths")
        kv_host = kv_host.cpu()
        batch_size = (
            metadata.q_cu_seq_lens.numel() - 1 if metadata.q_cu_seq_lens is not None else metadata.block_table.shape[0]
        )
        ...
        raise RuntimeError("dense MLA chunked prefill received invalid host KV sequence lengths")
