...
class KimiK3TextConfig:
    ...
    def validate(self) -> None:
        ...
        if self.ep_size > 1 and self.dp_size != self.ep_size:
            # Support two EP topologies:
            # 1. dp=1, attention_tp=ep (original)
            # 2. ep=dp*attention_tp (global EP for vLLM compatibility)
            supported = self.dp_size == 1 and self.tp_size == self.ep_size and self.world_size == self.ep_size
            supported = supported or (self.ep_size == self.dp_size * self.tp_size)
            if not supported:
                raise ValueError(
                    "Kimi K3 supports EP with either dp=ep or dp=1, attention_tp=ep, or ep=dp*attention_tp"
                )
