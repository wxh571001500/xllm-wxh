...
def _create_process_group(host: str, port: int, rank: int, world_size: int, device: str):
...
def init_tp_group(
    ...
    if registered_group is not None:
        if registered_group.local_rank != rank or registered_group.world_size != world_size:
            raise RuntimeError(
...
def _require_group(x: torch.Tensor, group_name: str) -> ParallelGroup:
    ...
    if group is not None:
        if group.world_size > 1 and group.process_group is None:
            raise RuntimeError(f"process group {group_name} for {x.device} has no backend")
        return group

    if group_name != "tp":
        raise RuntimeError(f"parallel group {group_name} was not initialized for {x.device}")

    legacy_group = _tp_groups.get(str(x.device))
    if legacy_group is None:
        raise RuntimeError(
            f"tensor-parallel collective called before the TP process group was initialized for {x.device}"
        )
...
def all_gather(
    ...
    if group.world_size != world_size:
        raise RuntimeError(f"{group_name} world-size mismatch: expected {world_size}, got {group.world_size}")
    if group.world_size == 1:
        return x.clone()
