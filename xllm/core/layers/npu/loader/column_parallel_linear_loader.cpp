...
void ColumParallelLinearLoader::fuse_eagle3_quarot_input_rotation(
  ...
  CHECK_EQ(weight.dim(), 2)
      << "Eagle3 fc weight must be 2D, got " << weight.sizes();
  ...
  CHECK_EQ(global_rotation.size(0), hidden_size)
      << "QuaRot global_rotation hidden size mismatch, expected " << hidden_size
      << ", got " << global_rotation.size(0);
  ...
  auto fused_weight =
      torch::cat(fused_chunks, /*dim=*/1)
          .to(torch::TensorOptions().dtype(weight_dtype).device(weight_device))
          .contiguous();
  weight = fused_weight;
}
