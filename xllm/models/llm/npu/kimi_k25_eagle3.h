...
REGISTER_MODEL_ARGS(kimi_k25_eagle3, [&] {
  ...
  LOAD_ARG_OR_FUNC(vocab_size, "vocab_size", [&] {
    return json.value_or<int64_t>("text_config.vocab_size", 163840);
  });
  ...
  LOAD_ARG_OR_FUNC(intermediate_size, "intermediate_size", [&] {
    return json.value_or<int64_t>("draft_config.intermediate_size", 18432);
  });
  ...
  LOAD_ARG_OR_FUNC(rms_norm_eps, "rms_norm_eps", [&] {
    return json.value_or<float>("draft_config.rms_norm_eps", 1e-5f);
  });
  ...
  LOAD_ARG_OR_FUNC(rope_theta, "rope_theta", [&] {
    return json.value_or<float>("text_config.rope_theta", 50000.0f);
  });
  ...
  LOAD_ARG_OR(max_window_layers, "max_window_layers", 1);
